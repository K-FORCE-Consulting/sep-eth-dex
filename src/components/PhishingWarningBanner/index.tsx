...
const domain = 'https://dex.k-forceglobal.com'
...
const PhishingWarningBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  const warningTextAsParts = useMemo(() => {
    ...
    return warningText.split(/(https:\/\/dex.k-forceglobal.com)/g)
  }, [t])
  const warningTextComponent = (
    <>
      <Text as="span" color="warning" small bold textTransform="uppercase">
    ...
    <Container className="warning-banner">
      {isMobile || isMd ? (
        ...
        <>
          <InnerContainer>
            <img
              src="/images/decorations/k-force.png"
              alt="phishing-warning"
              width="92px"
              onError={(e) => {
                const fallbackSrc = '/images/decorations/k-force.png'
                if (!e.currentTarget.src.endsWith(fallbackSrc)) {
                  // eslint-disable-next-line no-param-reassign
                  e.currentTarget.src = fallbackSrc
                }
